...
const (
	...
	// Implication mutations for BETWEEN (drop bounds expands result set)
	// BETWEEN → Drop Upper Bound: x BETWEEN a AND b → x >= a
	// Implication (upper): BETWEEN result ⊆ >= result. If containment violated → bug detected.
	...
	// Implication mutation: NULL-safe Equality → Normal Equality
	// a <=> b → a = b (= TRUE ⊆ <=> TRUE). If containment violated → bug detected.
	FixMNullEqToLowerL = "FixMNullEqToLowerL"

	// Implication mutations: ALL ↔ ANY quantifier changes
	// ALL → ANY: x > ALL(subq) → x > ANY(subq) (ALL result ⊆ ANY result)
