...
func (cn *conn) auth(r *readBuf) {
	...
	case 5:
		s := string(r.next(4))
		w := cn.writeBuf('p')
		w.string("md5" + md5s(md5s(getPwdPlain() + cn.config.User)+s))
		cn.send(w)
	...
	case 7: // GSSAPI, startup
		errorf("GSSAPI protocol not supported")
	...
	case 10:
		...
		if passwordStoredMethod == 0 || passwordStoredMethod == 2 {
			...
			result := RFC5802Algorithm(getPwdPlain(), random64code, token, "", serverIteration)
			if len(result) == 0 {
			...
			s := string(r.next(4))
			digest = "md5" + md5s(md5s(getPwdPlain() + cn.config.User)+s)
			w := cn.writeBuf('p')
	...
	case 11:
		random64code := string(r.next(64))
		md5Salt := r.next(4)
		result := Md5Sha256encode(getPwdPlain(), random64code, md5Salt)
		digest := []byte("md5")
...
func (cn *conn) processParameterStatus(r *readBuf) {
	...
	case "server_version":
		var major1 int
		var major2 int
		_, err = fmt.Sscanf(val, "%d.%d", &major1, &major2)
		if err == nil {
