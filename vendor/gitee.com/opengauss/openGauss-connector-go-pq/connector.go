...
func (c *Connector) connect(ctx context.Context, config *Config) (cn *conn, err error) {
	...
	for _, fc := range fallbackConfigs {
		...
		if err != nil {
			...
			config.Log(context.Background(), LogLevelDebug, err.Error(), map[string]interface{}{
		...
		config.Log(context.Background(), LogLevelDebug, "find instance", map[string]interface{}{
			"host": fc.Host, "port": fc.Port})
		break
	}
